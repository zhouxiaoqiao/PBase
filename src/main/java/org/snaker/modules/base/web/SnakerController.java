...
public class SnakerController
{
	private static final Logger log = LoggerFactory
			.getLogger(SnakerController.class);
	...
	public String homeTaskList(Model model)
	{
		...
		List<WorkItem> majorWorks = facets
				.getEngine()
				.query()
				.getWorkItems(
						majorPage,
						new QueryFilter().setOperators(assignees).setTaskType(
								TaskType.Major.ordinal()));
		List<WorkItem> aidantWorks = facets
				.getEngine()
				.query()
				.getWorkItems(
						aidantPage,
						new QueryFilter().setOperators(assignees).setTaskType(
								TaskType.Aidant.ordinal()));
		List<HistoryOrder> ccWorks = facets
				.getEngine()
				.query()
				.getCCWorks(ccorderPage,
	...
	public Map<String, String> addTaskActor(String orderId, String taskName)
	{
		...
		for (Task task : tasks)
		{
			if (task.getTaskName().equalsIgnoreCase(taskName))
			{
				String[] actors = facets.getEngine().query()
	...
	public String all(Model model, String processId, String orderId,
			String taskId, String type)
	{
		Process process = facets.getEngine().process()
		...
		if (StringUtils.isNotEmpty(process.getInstanceUrl()))
		{
			...
			{
				String sql = "select action_Url from wf_task where id='"
						+ taskId + "' and order_Id='" + orderId + "'";
				try
				{
					JSONObject ar;
					List<JSONObject> ret = db.executeJSONQuery(sql, null);
					if (ret != null)
					{
						ar = (JSONObject) ret.get(0);
						if (StringUtils.isNotEmpty(ar.getString("action_Url")))
							buffer.append("&action_Url=").append(
									ar.getString("action_Url"));
					}
				} catch (Exception e)
				{
		...
		if (StringUtils.isNotEmpty(orderId))
		{
			...
			List<HistoryTask> tasks = facets.getEngine().query()
					.getHistoryTasks(new QueryFilter().setOrderId(orderId));
			for (HistoryTask history : tasks)
			{
				model.addAttribute("variable_" + history.getTaskName(),
						history.getVariableMap());
			}
		}
		...
		for (WorkModel wm : models)
		{
			...
			if (StringUtils.isNotEmpty(formUrl) && formUrl.startsWith("forms/"))
			{
				...
				{
					Map<String, Object> formData = formDataManager.get(
							form.getTables(), orderId);
